 ...
 */
export function arrange(slots, prefs) {
    const byId = new Map(slots.map((slot) => [slot.id, slot]));
    const spoken = prefs.order.filter((id) => byId.has(id));
    const placed = spoken.map((id) => byId.get(id));
...
export function visibleRails(slots, prefs) {
...
export function moved(slots, prefs, id, by) {
    const list = arrange(slots, prefs).map((slot) => slot.id);
    const at = list.indexOf(id);
    const to = at + by;
    const swap = list[to];
    if (at < 0 || to < 0 || to >= list.length || swap === undefined)
        return list;
    list[at] = swap;
    list[to] = id;
    return list;
}
export function railsPage(client, signedIn, slots, prefs, 
/** True when nothing has been said, so Reset can say so. */
plain) {
    const off = new Set(prefs.off);
    const list = arrange(slots, prefs);
    const act = (what, id, label, enabled = true) => {
...
<span class="railacts">${act("up", slot.id, "&uarr; Up", index > 0 && !hidden)} ${act("down", slot.id, "&darr; Down", index < list.length - 1 && !hidden)} ${act(hidden ? "show" : "hide", slot.id, hidden ? "Show" : "Hide")}</span>
</li>`;
    })
        .join("\n");
    return page({
...
<p class="bar"><a class="step" href="${escape(client.link("/tv"))}">&lsaquo; Live TV</a> ${plain
            ? `<span class="step off">Reset</span>`
            : `<a class="step" href="${escape(`${client.link("/tv/rails")}?reset=1`)}">Reset</a>`}</p>
</div>
<p class="hint">${escape("This is for this television only. Rails the service adds later appear on their own -- hiding one here never hides a rail that does not exist yet.")}</p>
