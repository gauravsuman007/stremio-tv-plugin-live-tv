...
export function versionSupersedes(next, current) {
    if (!next)
        return false; // an unversioned file is never known to be an update.
    if (!current)
        return true; // versioned beats unversioned, unconditionally.
...
function ensureSourcesLoaded() {
    if (sourcesLoaded)
        return;
    sourcesLoaded = true;
    if (!config.githubSourcesStore)
        return;
    try {
        const parsed = JSON.parse(readFileSync(config.githubSourcesStore, "utf8"));
...
export async function importFromGithub(owner, repo, token) {
    ...
    if (!config.scrapersDir)
        throw new Error("SCRAPERS_DIR is not configured on this deployment");
...
export async function importFromStoredSource(owner, repo) {
    const source = findSource(owner, repo);
    if (!source)
        throw new Error("that source is not configured");
