/**
 * iptv-org's own curated, deduplicated JSON -- a real source, and this
 * plugin's DEFAULT scraper: it now lives here, in the live-tv plugin repo
 * itself, rather than in the separate stremio-tv-scrapers repository (a
 ...
 * this file's own `version` below is a real increase over what is on disk
 * -- see that module for the seed/update contract. If you want a similarly
 * structured scraper for a DIFFERENT source, copy this file and give it a
 * different id -- two scrapers sharing an id is refused everywhere a
