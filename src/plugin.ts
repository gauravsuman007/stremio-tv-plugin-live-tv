...
const createPlugin: PluginFactory = (host, configDir) => {
    ...
    */
    async function pullDefaultScraper(): Promise<void> {
        try {
            ...
            console.error(
                `live-tv: could not pull the default scraper from ${DEFAULT_SCRAPER_REPO} -- this is expected until that repository exists`,
                cause
            );
        }
    }
