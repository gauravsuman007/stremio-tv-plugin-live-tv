...
export interface ScraperRow {
    ...
    name: string;
    enabled: boolean;
    /** True for the one scraper this deployment cannot do without --
     *  offering to switch off the only source there is would just leave
     *  the page empty with no explanation. */
    sole: boolean;
    run: ScraperRun | null;
