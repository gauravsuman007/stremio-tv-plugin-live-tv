 ...
 */
function sweepLine(sweep: SweepState, action: string): string {
    const check = action
...
export interface LiveRow {
    /** See `Rail` in channels.ts: stable, so it can be arranged. */
    id: string;
    heading: string;
    by: string;
    channels: Channel[];
...
<p class="hint">${escape(
            "Ordered by what last night's check found first, then by how widely each channel is carried, what it broadcasts and whether it is a major in its own market -- a stand-in for popularity, not an audience figure."
        )}</p>`
    });
}
...
export function countryPage(
    ...
    const at = (from: number): string =>
        `${client.link(`/tv/country/${encodeURIComponent(country.code)}`)}${from ? `?skip=${from}` : ""}`;

    const pager =
        channels.length > perPage
            ? `<p class="bar">${
                  skip > 0
...
${
    shown.length
        ? `<p class="hint">${escape(
              `${skip + 1}-${skip + shown.length} of ${channels.length} channels, most widely carried first`
          )}</p>
<div class="wall">
...
export function channelSearchPage(
    ...
    const to = (term: string, key: string): string => {
        const parts: string[] = [];

        if (term) parts.push(`q=${encodeURIComponent(term)}`);
        if (key) parts.push(`k=${encodeURIComponent(key)}`);

        return `${client.link("/tv/search")}${parts.length ? `?${parts.join("&")}` : ""}`;
    };
    const ring = (key: string): string => (key === pressed ? ` id="tvfocus"` : "");

    const keys = KEYS().map(
        (letter) =>
            `<a class="key" data-term="${escape(query + letter)}"${ring(letter)} href="${escape(to(query + letter, letter))}">${escape(letter)}</a>`
    ).join("\n");

    const spaced = query && !/ $/.test(query) ? `${query} ` : query;

    const edits =
        `<a class="key key-wide" data-term="${escape(spaced)}"${ring("space")} href="${escape(to(spaced, "space"))}">Space</a>` +
    ...
    const results = waiting
        ? `<p class="empty">${
              query ? "Keep going &mdash; two letters is enough." : "Pick letters on the left."
          }</p>`
        : channels.length
          ? `<div class="wall">
${channels.map((channel) => chanCard(client, { ...channel, note: describeChannel(channel) })).join("\n")}
