...
export interface SweepState {
    running: boolean;
    /** When the last sweep finished, or 0. */
    at: number;
    tried: number;
    found: number;
    /** Sources followed all the way through to a segment of real video. */
    deep?: number;
}

export function sweepState(): SweepState {
    if (lastRun) return { running, at: lastRun, tried: lastTried, found: lastFound, deep: lastDeep };
...
export async function sweep(): Promise<void> {
    ...
    */
    ...
    const queues = new Map<string, ChannelStream[]>();
    ...
    */
    ...
    console.log(
        `stremio-tv: live sweep starting -- ${built.all.length} channels, ` +
    ...
    */
    ...
    const ticker = setInterval(() => {
        console.log(
            `stremio-tv: live sweep ${tried}/${total} sources, ${found} answered, ` +
    ...
    async function worker(): Promise<void> {
        for (;;) {
            if (halted || Date.now() > deadline) return;
            ...
            for (const stream of queue) {
                ...
                try {
                    if (await verify(stream, proxy)) found += 1;
                } catch {
    ...
    if (halted || Date.now() > deadline) {
        console.error(
            `stremio-tv: live sweep hit its ${Math.round(WHOLE_SWEEP_MS / 60_000)} min deadline ` +
    ...
    */
    ...
    console.log(
        `stremio-tv: live sweep done -- ${found} of ${tried} sources answered, in ${minutes} min` +
            (deepened.tried
 ...
 */
export async function nameCodecs(
    built: { all: Channel[] },
    proxy: string,
    deadline: number
): Promise<number> {
    ...
    for (const channel of built.all) {
        ...
        */
        ...
        if (held && Date.now() - held.at < CODEC_FRESH_MS) continue;
    ...
    */
    const age = (stream: ChannelStream): number => (codecFor(stream.url) || { at: 0 }).at;
    ...
    */
    const byHost = new Map<string, ChannelStream[]>();
    ...
    console.log(
        `stremio-tv: naming codecs for ${queue.length} leading sources` +
    ...
    async function worker(): Promise<void> {
        for (;;) {
            ...
            if (await probeCodec(stream, proxy)) named += 1;
...
export async function deepen(
    built: { all: { id: string; streams: ChannelStream[] }[] },
    proxy: string,
    deadline: number
): Promise<{ tried: number; proven: number }> {
    const wanted = new Set(allFavourites());
    const first: ChannelStream[] = [];
    const rest: ChannelStream[] = [];
    ...
    if (hosts.length === 0) return { tried: 0, proven: 0 };
    ...
    console.log(
        `stremio-tv: deep check starting -- ${first.length + rest.length} live sources ` +
    ...
    async function worker(): Promise<void> {
        for (;;) {
            ...
            for (const stream of queue) {
                ...
                try {
                    if (await deepVerify(stream, proxy)) proven += 1;
                } catch {
...
export function scheduleSweep(): void {
    ...
    console.log(
        `stremio-tv: nightly live sweep at ${String(config.liveSweepHour).padStart(2, "0")}:00` +
